...
export const PATCH = async (request: Request) => {
    try {
        ...
        if (!userId || !newUsername) {
            return new NextResponse(JSON.stringify({message: "ID or new username invalid"}), {status: 400})
        }
        ...
        if (!updatedUser) {
        return new NextResponse(JSON.stringify({message: "User not updated"}), {status: 400})
        }
...
export const DELETE = async (request: Request) => {
    try {
        ...
        if (!deletedUser) {
            return new NextResponse(JSON.stringify({ message: "User not deleted" }), { status: 400 })
        }
